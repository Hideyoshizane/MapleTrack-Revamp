...
import { BossList } from './bossListModel';

import type { BossListDocument, BossCharacter, BossServer } from './bossListModel';
...
export const addCharacterToBossList = async (
	...
	if (!server) {
		throw new Error(`Failed to create or find server ${serverName} in BossList.`);
	}
...
export const removeCharacterFromBossList = async (
	...
	try {
		...
		if (updatedCharacter.length === server.characters.length) {
			return;
		}
