...
import { nowInUtc, addMinutesToDate } from '@utils/time';
...
export const POST = async (request: NextRequest): Promise<NextResponse> => {
	try {
		...
		const resetPasswordExpiresInstant = addMinutesToDate(nowInUtc(), 15);
		const resetPasswordExpires = resetPasswordExpiresInstant;
